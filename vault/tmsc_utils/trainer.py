...
from vault.train_utils import EarlyStopping
...
class Twitter201XTrainer(ABC):
        ...
        for epoch in range(num_epochs):
            ...
            for step, batch in enumerate(batch_itr):
                ...
                loss = self.calculate_loss(
                    logits, self.batch_labels(batch), train=True
                )
                ...
                train_loss += loss.item() * self.batch_len(batch)
    ...
    def evaluate(
        ...
        for batch in batch_itr:
            ...
            loss = self.calculate_loss(logits, labels, train=False)
            if loss is not None:
                eval_loss += loss.item() * self.batch_len(batch)

            eval_preds.extend(self.get_eval_preds_from_batch(logits))
            eval_true.extend(self.get_eval_true_from_batch(labels))

        ### compute eval metrics
        eval_loss /= len(data_loader.dataset)

        results = dict(eval_loss=eval_loss)
        results.update(
            self.evaluation_metrics(
                eval_true, eval_preds, data_loader=data_loader
            )
        )
